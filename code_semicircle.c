// generating mesh to capture boundary layer
...
// changing circle calculation ...making it angle based 
// improving value of PI from 3.1416 to 3.1415926535897932384626433832792 
...
double   DI_D=50.0 ; /* Ratio of intermediate boundary to inner boundary */
double   DI_D1=0.5 ; /* ratio of intermediate boundary to outer boundary */
double   angle= PI, angle1= 0.0 ; 
double   stretching=1.0 ;
long int Imax=10, Jmax=10 ; //Imax 
...
void    generate_refinedmesh(void) ;
...
int main(int argc, char *argv[])
{
    ...
    printf("\n") ;
    printf("Setting default mesh size in single block \n") ;
    Block_num = 1 ;
    ...
    {
        printf("\n") ;
        printf("generating refined mesh \n") ;
        generate_refinedmesh() ;
    }
    ...
    output_plot3D() ;
//  printf("Writing plot3D binary file \n") ;
//  output_plot3DBinary() ;
...
void compute_Jmax()
{
    int i, count=0, do_loop=1, do_iloop=1 ;
    double dtheta, delr, distance ;
        ...
        if(BLFlag == 1)
        {
        for(i=3; i<=N1 && do_iloop==1 ; i++)
        {
            if(dr0*pow(S,i-2) < (R+distance)*dtheta)
            {
              if(R+distance+dr0*pow(S,i-2) < R1)
              {
                count++ ;
                delr = dr0*pow(S,i-2) ;
                distance = distance + delr ;
...
void generate_refinedmesh()
{
    int     i,j, count=0 ;
    double  *LinearMeshRefined ;
    double  zi ;

    LinearMeshRefined = (double *) malloc ((GRMultiple*(Jmax-1)+1)*sizeof(double)) ;
    for(i=0; i<Jmax-1; i++)
    {
        LinearMeshRefined[i*GRMultiple] = LinearMesh[i] ;
        for(j=1; j<GRMultiple; j++)
        {
            zi = j*1.0/GRMultiple ;
            LinearMeshRefined[i*GRMultiple+j] = LinearMesh[i] + zi*(LinearMesh[i+1]-LinearMesh[i]) ;
        }
    }
          
    LinearMeshRefined[(Jmax-1)*GRMultiple] = LinearMesh[Jmax-1] ;
    
    /* free LinearMesh */
    free(LinearMesh) ;

    /* reset mesh size */
    Imax = GRMultiple*(Imax-1)+1 ;
    Jmax = GRMultiple*(Jmax-1)+1 ;

    Blocks[0].Imax = Imax ; 
    Blocks[0].Jmax = Jmax ;
    Blocks[0].boundary[0].Imax = Blocks[0].Imax ;
    Blocks[0].boundary[2].Imax = Blocks[0].Imax ;
    Blocks[0].boundary[1].Imax = Blocks[0].Jmax ;
    Blocks[0].boundary[3].Imax = Blocks[0].Jmax ;

    /* again allocate LinearMesh and copy RefinedLinearMesh to LinearMesh */
    LinearMesh = (double *) malloc (Jmax*sizeof(double)) ;

    for(i=0; i<Jmax; i++)
    {
        LinearMesh[i] = LinearMeshRefined[i] ;
    }

    /* free LinearMeshRefined */
    free(LinearMeshRefined) ;
...
{
    int i,j, count=0, do_loop=1, need_smoothening=0, stencil_size=10 ;
    int do_iloop=1, need_ismoothening=0, istencil_size=5 ;
    ...
    LinearMesh = (double *) malloc (Jmax*sizeof(double)) ;
        ...
        if(BLFlag == 1)
        {
          ...
          for(i=3; i<=N1 && do_iloop==1 ; i++)
          {
            if(dr0*pow(S,i-2) < (R+distance)*dtheta)
            {
              ...
              {
                printf("same stretching ratio continued throughout \n") ;
                count++ ;
                delr = R1-(R+distance) ;
                distance = R1-R ;
                LinearMesh[count-1] = R1 ;
                do_loop = 0 ;
                do_iloop = 0 ;
                need_ismoothening = 0 ;
                need_smoothening = 1 ;
            ...
            {
              printf("\nswitching over to second algorithm \n") ;
              printf("count=%d r=%1.16e\n",count,LinearMesh[count-1]) ;
              need_ismoothening = 0 ;
              do_iloop = 0 ;
              exit ; 
            }
        ...
        /** smoothen the mesh at interface **/
        if(need_ismoothening == 1)
        {
           istencil_size = N1/5 ;
           if(istencil_size > 10 ) istencil_size = 5 ; 
           if(istencil_size < 3 ) istencil_size = 3 ; 
           if(istencil_size >= N1 ) need_ismoothening = 0 ; 
        }
        if(need_ismoothening == 1)
        {
           r1 = LinearMesh[N1-istencil_size-1] ;
           r2 = LinearMesh[N1+istencil_size+1] ;

           for(i=N1-istencil_size; i<=N1+istencil_size; i++)
           {
             zi = (i*1.0 - N1+istencil_size+1)/(2*istencil_size+2.0) ;
...
void impose_symmetry()
{
    ...
    /*** imposing symmetry in first quadrant and second quadrant ********/
    for(i=0; i<Imax; i++)
    {
        for(j=0; j<Jmax; j++)
        {
    ...
    /*** imposing x=0 on center line ********/
    if(Imax%2 != 0)
    {
        for(j=0; j<Jmax; j++)
        {
            Blocks[0].grid[(Imax)/2][j].x = 0.0 ;
        }
    }

    /*** imposing y=0 on symmetry boundaries (i=0, i=Imax-1) ********/
    i = 0 ; 
    for(j=0; j<Jmax; j++)
    {
      Blocks[0].grid[i][j].y = 0.0 ;
    }
    i = Imax-1 ;    
    for(j=0; j<Jmax; j++)
    {
        ...
        case 8 :
                ...
                if(mode == 0)
                {
                  ...
                  printf("mode=%d,  D=%1.4e,  D1_D=%1.4e,  Imax=%d  \n",
                        mode,D,D1_D,gpts) ;
                  BLFlag = 1 ; 
                  printf("Boundary-Layer grid would be generated") ;
                  printf("and also delta-r=%1.4e S=%1.4e N1=%d\n",dr0,S,N1) ;
                }
                break ;
        case 9 :
                mode = atoi(argv[1]) ;
                ...
                printf("grid will be generated by a factor of %d in each direction\n",GRMultiple) ;
                GRFlag = 1 ;
                break ;
        default :
...
//  free(Blocks[block].boundary[side].edgegrid) ;
    ...
    if(!(Blocks[block].boundary[side].edgegrid = (vertex *) malloc (Blocks[block].boundary[side].Imax*sizeof(vertex))))
    {
...
//  free(Blocks[0].grid) ;
    ...
    if(!(Blocks[block].grid = (vertex **) malloc (Blocks[block].Imax*sizeof(vertex *))))
    {
    ...
    {
        if(!( Blocks[block].grid[j] = (vertex *) malloc (Blocks[block].Jmax*sizeof(vertex))))
        {
...
void output_format()
{
    ...
    nEdges1 = nEdges2 = nEdges3 = 0 ;

    nEdges1 += Blocks[0].Imax - 1 ;
    nEdges2 += Blocks[0].Imax - 1 ;
    nEdges3 += 2*(Blocks[0].Jmax - 1) ;
    ...
    fprintf(fp,"%d %d\n",btype3,nEdges3) ;
    i = 0 ; ii = 0 ;
    for(jj=Blocks[i].Jmax-1; jj>0; jj--)
    {
        v1 = Blocks[i].grid[ii][jj].vertexno ;
        v2 = Blocks[i].grid[ii][jj-1].vertexno ;
        fprintf(fp,"%d %d\n",v1,v2) ;
    }
    i = 0 ; ii = Blocks[i].Imax - 1 ;
    for(jj=0; jj<Blocks[i].Jmax-1; jj++)
    {
        v1 = Blocks[i].grid[ii][jj].vertexno ;
        v2 = Blocks[i].grid[ii][jj+1].vertexno ;
...
/*********************** output_plot3DBinary ***********************************************/
void output_plot3DBinary()
{
    long int        i, j, ii, jj, M, N, count ;
    FILE            *fp, *f1, *f2 ;
    int             Kmax, num, nCells, nVerts ;
    int             v1, v2, v3, v4 ;
    int             izero ;
    double          dzero,z ;
...
//   fwrite(&aa, sizeof(aa), 1, f1);
//   fwrite(&bb, sizeof(bb), 1, f1);
...
/***********************************/
    ...
    Kmax = 2 ;
    fwrite(&(Blocks[0].Imax),sizeof(int),1,fp) ;
    fwrite(&(Blocks[0].Jmax),sizeof(int),1,fp) ;
    fwrite(&Kmax,sizeof(int),1,fp) ;
    printf("%d %d %d\n",Blocks[0].Imax,Blocks[0].Jmax,Kmax) ;
    ...
    for(ii=0; ii<Blocks[i].Imax; ii++)
    {
            fwrite(&(Blocks[i].grid[ii][jj].x),sizeof(double),1,fp) ;
    }
    for(jj=0; jj<Blocks[i].Jmax; jj++)
    for(ii=0; ii<Blocks[i].Imax; ii++)
    {
    ...
    for(ii=0; ii<Blocks[i].Imax; ii++)
    {
            fwrite(&(Blocks[i].grid[ii][jj].y),sizeof(double),1,fp) ;
    }
    for(jj=0; jj<Blocks[i].Jmax; jj++)
    for(ii=0; ii<Blocks[i].Imax; ii++)
    {
